...
def _causal_forest_importance(
    ...
    Besonderheiten
    --------------
    - GRF kann keine fehlenden Werte verarbeiten — ``compute_importances()``
      prüft dies vorab und überspringt die Methode automatisch.
    - X wird intern zu numpy float64 konvertiert (GRF verarbeitet keine
      pandas category-Dtypes).
    - Bei Multi-Treatment (T ∈ {0,1,...,K-1}) wird T automatisch binarisiert
      (Control vs. Any Treatment), da GRF binäre Treatments erwartet.
    - CausalForest nutzt intern joblib-Prozesse für Baum-Parallelisierung.
      Um fork-Probleme zu vermeiden (OpenMP-Threads von vorherigen LightGBM-Fits,
      MLflow-Threads, etc.), wird n_jobs=1 erzwungen. 100 Bäume sequentiell
    ...
    # ── Fit ──
    # n_jobs=1 erzwungen: GRF nutzt intern joblib-Prozesse (loky/fork),
    # die mit bestehenden OpenMP-Threads (LightGBM) oder MLflow-Threads
    # kollidieren können. 100 Bäume sequentiell: ~3-8s.
    cf = CausalForest(
        n_estimators=100,
        min_samples_leaf=20,
        random_state=seed,
    ...
    _logger.info(
        "CausalForest FS: fit(%d×%d, T unique=%d, n_estimators=100, n_jobs=1)...",
        X_np.shape[0], X_np.shape[1], len(np.unique(T_np)),
    )
...
def compute_importances(
    ...
    if parallel_methods and len(effective_methods) >= 2:
        # Methoden parallel: jede Methode bekommt anteilige Kerne.
        # CausalForest nutzt intern n_jobs=1 (kein joblib-Fork), daher ist
        # Thread-Parallelisierung mit LightGBM-Methoden sicher.
        import os
        ...
        # Module VOR Thread-Start laden (Python Import-Lock-Sicherheit)
