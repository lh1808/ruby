...
## 14) Feature-Selektion: Parallele Methoden

`compute_importances()` parallelisiert Methoden bei Level 3/4 via joblib-Threads. CausalForest nutzt intern `n_jobs=1` (erzwungen), da GRFs joblib-basierte Baum-Parallelisierung sonst Fork-Deadlocks in Threads verursacht. LightGBM-Methoden nutzen OpenMP (thread-safe). Dadurch können alle Methodenkombinationen parallel laufen:

- Thread 1: `lgbm_importance` (OpenMP, n_jobs=Kerne/2)
- Thread 2: `causal_forest` (single-threaded, n_jobs=1)

Bei Multi-Treatment binarisiert `_causal_forest_importance()` T automatisch zu Control(0) vs. AnyTreatment(1), da GRF binäres Treatment erwartet.
...
- **Feature-Selektion** (`_causal_forest_importance`): `n_jobs=1` erzwungen — GRF läuft single-threaded, LightGBM parallel daneben
