...
**Parallelisierung:** Bei `parallel_level` 3–4 werden die Importance-Methoden parallel in Threads ausgeführt (z. B. `lgbm_importance` + `causal_forest` gleichzeitig). LightGBM nutzt dabei OpenMP-Threads, CausalForest läuft single-threaded (n_jobs=1), da GRF intern joblib-Prozesse verwenden würde, die in Threads zu Deadlocks führen. Bei Level 1–2 laufen die Methoden sequentiell mit voller Kernauslastung pro Methode.
