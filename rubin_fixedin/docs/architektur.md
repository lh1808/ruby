...
Pro Methode werden die Top-X% behalten, dann per Union vereinigt. Anschließend Korrelationsfilter (Pearson + Spearman). Die LightGBM-Fits nutzen automatisch native kategoriale Splits (via `categorical_feature`-Patch). Bei Level 3–4 werden die Methoden parallel in Threads ausgeführt. LightGBM nutzt OpenMP, CausalForest läuft intern single-threaded (n_jobs=1 erzwungen, da GRF-internes joblib sonst Fork-Deadlocks verursacht). Bei Multi-Treatment wird T für GRF automatisch binarisiert (Control vs. Any Treatment).
