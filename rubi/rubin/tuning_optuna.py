...
class BaseLearnerTuner:
    ...
    def _tuning_n_jobs(self) -> int:
        ...
        Level 1-2: 1 (sequentiell, alle Kerne an den einzelnen LightGBM-Fit)
        Level 3:   2-4 parallele Trials (Kerne aufgeteilt)
        Level 4:   max. parallele Trials (Kerne aufgeteilt)
        """
        pl = self.cfg.constants.parallel_level
        if pl <= 2:
            return 1
        ...
        n_cpus = os.cpu_count() or 1
        if pl >= 4:
            # Level 4: so viele parallele Trials wie sinnvoll (min. 2 Kerne pro Trial)
...
persistiert und in weiteren Folds wiederverwendet."""
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            study.optimize(objective, n_trials=int(self.cfg.final_model_tuning.n_trials), n_jobs=self._tuning_n_jobs())
            try:
