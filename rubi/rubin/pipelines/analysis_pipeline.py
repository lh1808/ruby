...
class AnalysisPipeline:
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow):
        ...
        # ── Phase 3: scikit-uplift Plots für Modelle OHNE DRTester-Diagnostik ──
        # sklift-Plots sind schnell (~2-5s) und werden IMMER für alle Modelle erstellt.
        ...
        for mname, dfp in preds.items():
            if mname in plot_models:
                continue  # DRTester hat sklift bereits erzeugt
            y = dfp["Y"].to_numpy()
            t = dfp["T"].to_numpy()
            if is_mt:
                mt_pred_cols = [c for c in dfp.columns if c.startswith(f"Predictions_{mname}_T")]
