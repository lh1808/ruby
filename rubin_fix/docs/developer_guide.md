...
```python
with patch_categorical_features(X, base_learner_type="lgbm") as cat_indices:
    # Innerhalb: LGBMClassifier.fit und LGBMRegressor.fit sind gepatcht
    # categorical_feature=cat_indices wird bei jedem .fit()-Aufruf übergeben
    tuner.tune_all(...)
    model.fit(Y, T, X=X)
# Außerhalb: Originale .fit()-Methoden wiederhergestellt
```

Der Patch deckt LightGBM (`categorical_feature`) und CatBoost (`cat_features`) ab. Er wird in `analysis_pipeline.run()` um den gesamten ML-Kern gelegt (Tuning → Training → Evaluation → Surrogate → Bundle-Export).
