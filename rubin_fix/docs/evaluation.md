...
In der Analyse-Pipeline werden zwei Ebenen der Evaluation unterschieden:

1. **Sortierungsmetriken** (Qini, AUUC, Uplift@k, Policy Value)
   - basieren auf **Cross-Predictions** (Out-of-Fold)
   - sind die wichtigste Grundlage für den Vergleich von Scores, weil sie
     direkt die Qualität der Rangfolge („wen behandle ich zuerst?“) bewerten

2. **Diagnose-Plots und Zusatztests** (EconML `DRTester` und scikit-uplift)
   - werden zusätzlich erzeugt, wenn die benötigten Pakete installiert sind
   - nutzen ebenfalls die bereits berechneten Cross-Predictions
...
Zusätzlich werden Plots aus scikit-uplift erzeugt:
