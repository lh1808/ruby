...
def _build_lgbm_classifier(params: Dict[str, Any], seed: int):
...
def _build_lgbm_regressor(params: Dict[str, Any], seed: int):
    import lightgbm as lgbm
    fixed = dict(
        random_state=seed,
        n_jobs=-1,
...
def _build_catboost_classifier(params: Dict[str, Any], seed: int):
...
def _build_catboost_regressor(params: Dict[str, Any], seed: int):
...
def build_base_learner(base_type: str, params: Dict[str, Any], seed: int, task: str = "classifier"):
...
dass identische Defaults (z. B. Random Seed) gesetzt werden."""
    ...
    if base_type == "lgbm":
        return _build_lgbm_classifier(params, seed) if task == "classifier" else _build_lgbm_regressor(params, seed)
    if base_type == "catboost":
        return _build_catboost_classifier(params, seed) if task == "classifier" else _build_catboost_regressor(params, seed)
...
def _default_search_space(base_type: str) -> Dict[str, SearchSpaceParameterConfig]:
    ...
    if base_type == "lgbm":
        return {
            ...
            "subsample_freq": SearchSpaceParameterConfig(type="int", low=1, high=7),
            "colsample_bytree": SearchSpaceParameterConfig(type="float", low=0.5, high=1.0),
            "min_split_gain": SearchSpaceParameterConfig(type="float", low=0.0, high=1.0),
            "reg_alpha": SearchSpaceParameterConfig(type="float", low=1e-8, high=10.0, log=True),
            "reg_lambda": SearchSpaceParameterConfig(type="float", low=1e-8, high=10.0, log=True),
        }
    if base_type == "catboost":
        return {
            ...
            "bagging_temperature": SearchSpaceParameterConfig(type="float", low=0.0, high=5.0),
            "subsample": SearchSpaceParameterConfig(type="float", low=0.5, high=1.0),
            "rsm": SearchSpaceParameterConfig(type="float", low=0.5, high=1.0),
            "min_data_in_leaf": SearchSpaceParameterConfig(type="int", low=1, high=128),
            "leaf_estimation_iterations": SearchSpaceParameterConfig(type="int", low=1, high=10),
            "border_count": SearchSpaceParameterConfig(type="int", low=32, high=255),
        }
    raise ValueError(f"Unbekannter base_type {base_type}")


def _search_space_for(base_type: str, cfg_space: SearchSpaceConfig | None) -> Dict[str, SearchSpaceParameterConfig]:
    base_type = (base_type or "").lower()
    custom = {}
    if cfg_space is not None:
        custom = dict(getattr(cfg_space, base_type, {}) or {})
    return custom or _default_search_space(base_type)
...
def _suggest_params(trial, base_type: str, cfg_space: SearchSpaceConfig | None = None) -> Dict[str, Any]:
    base_type = base_type.lower()
    specs = _search_space_for(base_type, cfg_space)
    params: Dict[str, Any] = {}
...
class BaseLearnerTuner:
    ...
    def _create_study(self, study_key: str):
        ...
        try:
            sampler = optuna.samplers.TPESampler(seed=int(self.cfg.tuning.optuna_seed))
        except Exception:
            sampler = None
    ...
    def _score_classifier(self, y_true: np.ndarray, proba: np.ndarray, multiclass: bool = False) -> float:
        ...
        try:
            if multiclass or len(np.unique(y_true)) > 2:
                # Multiclass: proba ist (n, K), roc_auc_ovr
    ...
    def _fit_model(self, params: Dict[str, Any], X_train: np.ndarray, y_train: np.ndarray, estimator_task: str):
        model = build_base_learner(self.cfg.base_learner.type, params, seed=self.seed, task=estimator_task)
...
persistiert und in weiteren Folds wiederverwendet."""
    ...
    def _build_classifier(self, base_type: str, fixed: Dict[str, Any], tuned: Dict[str, Any]) -> Any:
        ...
        return build_base_learner(base_type, params, seed=self.seed, task="classifier")

    def _build_regressor(self, base_type: str, fixed: Dict[str, Any], tuned: Dict[str, Any]) -> Any:
        params = dict(fixed or {})
        params.update(tuned or {})
        return build_base_learner(base_type, params, seed=self.seed, task="regressor")
    ...
    def tune_final_model(
        ...
        if name == "nonparamdml":
            ...
            def objective(trial):
                cand_params = _suggest_params(trial, base_type, self.cfg.final_model_tuning.search_space)
                est = NonParamDML(
                    model_y=self._build_classifier(base_type, base_fixed_params, tuned_roles.get("model_y", {})),
                    model_t=self._build_classifier(base_type, base_fixed_params, tuned_roles.get("model_t", {})),
                ...
                return float(scorer.score(est))

            study.optimize(objective, n_trials=int(self.cfg.final_model_tuning.n_trials))
            try:
                self.best_scores[f"final__{model_name}"] = float(study.best_value)
            except Exception:
                pass
        ...
        if name == "drlearner":
            ...
            def objective(trial):
                ...
                for tr, va in split_iter:
                    ...
                    # EconML DRLearner.score() gibt einen R²-artigen Wert zurück
                    # (höher = besser), daher keine Negierung nötig.
