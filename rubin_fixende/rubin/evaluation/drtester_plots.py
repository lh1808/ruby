...
def _native_uplift_by_percentile(
    ...
    """Eigener Uplift-by-Percentile-Plot im rubin-Farbschema.

    Ersetzt ``sklift.viz.plot_uplift_by_percentile``, das mit numpy >=1.24
    wegen inhomogener Arrays crasht."""
    try:
        order = np.argsort(uplift)[::-1]
        y_s, t_s = y_true[order], treatment[order]
        n = len(y_s)
        bin_size = n // n_bins
        ...
        for i in range(n_bins):
            ...
            treat_mask, ctrl_mask = t_bin == 1, t_bin == 0
            if treat_mask.sum() > 0 and ctrl_mask.sum() > 0:
                u = y_bin[treat_mask].mean() - y_bin[ctrl_mask].mean()
            else:
                u = 0.0
            uplifts.append(u)
            pct_lo = int(round(lo / n * 100))
            pct_hi = int(round(hi / n * 100))
            labels.append(f"{pct_lo}–{pct_hi}%")

        fig, ax = plt.subplots(figsize=(10, 5))
        colors = [RUBIN_COLORS["ruby"] if u >= 0 else RUBIN_COLORS["slate"] for u in uplifts]
        ax.bar(range(n_bins), uplifts, color=colors, edgecolor=RUBIN_COLORS["ruby_dark"],
               linewidth=0.5, alpha=0.85)
        ax.set_xticks(range(n_bins))
        ax.set_xticklabels(labels, rotation=45, ha="right", fontsize=9)
        ax.axhline(0, color=RUBIN_COLORS["slate"], linewidth=0.8, linestyle="--")
        ax.set_xlabel("Percentile Bucket (sortiert nach pred. Uplift)", fontsize=10)
        ax.set_ylabel("Observed Uplift (E[Y|T=1] − E[Y|T=0])", fontsize=10)
        ax.set_title("Uplift by Percentile", fontsize=12, fontweight="bold",
...
def _native_treatment_balance(
    ...
    """Eigener Treatment-Balance-Plot im rubin-Farbschema.

    Ersetzt ``sklift.viz.plot_treatment_balance_curve``, das mit numpy >=1.24
    wegen inhomogener Arrays crashen kann."""
    try:
        order = np.argsort(uplift)[::-1]
        t_s = treatment[order]
        n = len(t_s)
        ...
        for i in range(n_bins):
            lo = i * bin_size
            hi = (i + 1) * bin_size if i < n_bins - 1 else n
            fracs.append(float(t_s[lo:hi].mean()))
            pct_lo = int(round(lo / n * 100))
            pct_hi = int(round(hi / n * 100))
            labels.append(f"{pct_lo}–{pct_hi}%")

        fig, ax = plt.subplots(figsize=(10, 5))
        ax.bar(range(n_bins), fracs, color=RUBIN_COLORS["ruby_light"],
               edgecolor=RUBIN_COLORS["ruby_dark"], linewidth=0.5, alpha=0.85)
        ax.axhline(overall_rate, color=RUBIN_COLORS["ruby"], linewidth=1.5,
                   linestyle="--", label=f"Overall Treatment Rate ({overall_rate:.1%})")
        ax.set_xticks(range(n_bins))
        ax.set_xticklabels(labels, rotation=45, ha="right", fontsize=9)
        ax.set_xlabel("Percentile Bucket (sortiert nach pred. Uplift)", fontsize=10)
        ax.set_ylabel("Treatment Rate", fontsize=10)
        ax.set_title("Treatment Balance", fontsize=12, fontweight="bold",
                      color=RUBIN_COLORS["ruby_dark"])
        ax.legend(fontsize=9)
        ax.set_ylim(0, min(1.0, max(fracs) * 1.3))
        fig.tight_layout()
        return fig
    except Exception as e:
...
def generate_sklift_plots(
    ...
    """Erzeugt Uplift-Plots: Qini-Kurve (sklift), Uplift-by-Percentile und
    Treatment-Balance (eigene Implementierung, numpy >=1.24 kompatibel).
    ...
    # Qini-Kurve: sklift funktioniert hier zuverlässig
    if _SKLIFT_AVAILABLE:
        try:
            ...
            recolor_figure(sk_qini)
        except Exception as e:
            _logger.warning("sklift Qini-Curve fehlgeschlagen: %s", e)

    # Uplift-by-Percentile: Eigene Implementierung (sklift crasht mit numpy >=1.24)
    sk_pct = _native_uplift_by_percentile(y_val_arr, uplift, t_val_arr)

    # Treatment-Balance: Eigene Implementierung (sklift crasht mit numpy >=1.24)
...
def evaluate_cate_with_plots(
    *,
    ...
    # Qini-Kurve: sklift funktioniert zuverlässig
    if _SKLIFT_AVAILABLE:
        try:
            result = plot_qini_curve(
                y_true=y_val_arr, uplift=uplift, treatment=t_val_arr, perfect=False,
    ...
    # Uplift-by-Percentile + Treatment-Balance: Eigene Implementierung
    sk_pct = _native_uplift_by_percentile(y_val_arr, uplift, t_val_arr)
    sk_tb = _native_treatment_balance(uplift, t_val_arr)
