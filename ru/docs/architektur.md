...
Pro Methode werden die Top-X% behalten, dann per Union vereinigt. Anschließend Korrelationsfilter (Pearson + Spearman). Die LightGBM-Fits nutzen automatisch native kategoriale Splits (via `categorical_feature`-Patch). Bei Level 3–4 werden die Methoden parallel ausgeführt, wobei die CPU-Kerne proportional aufgeteilt werden.
