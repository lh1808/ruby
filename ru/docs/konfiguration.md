...
Bei Level 3–4 werden die CPU-Kerne proportional aufgeteilt — sowohl für parallele Optuna-Trials im Tuning als auch für parallele CV-Folds im Training. Keine Übersubskription (z. B. 16 Kerne, Level 4: 8 parallele Trials × 2 Kerne = 16 aktiv). Die Feature-Selektion respektiert ebenfalls den parallel_level (Level 1 → `n_jobs=1`).

**Evaluation-Optimierung:** Die schnellen Metriken (Qini, AUUC, Policy Value) werden immer für alle Modelle berechnet — unabhängig vom Level. Die teuren DRTester-Diagnostik-Plots (Calibration, Qini/TOC mit Bootstrap-CIs) werden bei Level 3 nur für Champion + besten Challenger erzeugt, bei Level 4 nur für den Champion. Die scikit-uplift-Plots (Qini-Kurve, Uplift-by-Percentile, Treatment-Balance) werden immer für alle Modelle erstellt. Die DRTester-Nuisance-Modelle nutzen leichtere Varianten (n_estimators≤100, cv=3) für schnelleres Fitting.
...
**Parallelisierung:** Bei `parallel_level` 3–4 werden die Importance-Methoden parallel ausgeführt (z. B. `lgbm_importance` + `causal_forest` gleichzeitig). Die CPU-Kerne werden proportional aufgeteilt. Bei Level 1–2 laufen die Methoden sequentiell mit voller Kernauslastung pro Methode.
