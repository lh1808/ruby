...
def _causal_forest_importance(
    ...
    return pd.Series(imp, index=X.columns, name="causal_forest").sort_values(ascending=False)
...
def compute_importances(
    ...
    for method in methods:
        if method == "none":
            continue
        # GRF/CausalForest kann keine fehlenden Werte verarbeiten.
        ...
        try:
            if method == "lgbm_importance":
                results[method] = _lgbm_gain_importance(X, Y, seed, n_jobs=n_jobs)
            elif method == "lgbm_permutation":
                results[method] = _lgbm_permutation_importance(X, Y, seed, n_jobs=n_jobs)
            elif method == "causal_forest":
                results[method] = _causal_forest_importance(X, T, Y, seed, n_jobs=n_jobs)
            else:
