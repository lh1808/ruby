...
class AnalysisPipeline:
    ...
    def _run_tuning(self, cfg, X, T, Y, mlflow):
        ...
        if getattr(cfg, "final_model_tuning", None) is not None and cfg.final_model_tuning.enabled:
            ...
            for task_key, score in final_tuner.best_scores.items():
                mlflow.log_metric(f"tuning_best__{task_key}", score)
            if hasattr(self, '_report'):
                self._report.tuning_scores.update(final_tuner.best_scores)
                # FMT-Plan für Report
                try:
    ...
    def _run_evaluation(self, cfg, X, T, Y, S, holdout_data, preds, models, tuned_params_by_model, mlflow):
        ...
        try:
            model_reg = build_base_learner(cfg.base_learner.type, _best_clf_params_y, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=1 if cfg.constants.parallel_level <= 1 else -1)
            model_prop = build_base_learner(cfg.base_learner.type, _best_clf_params_t, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=1 if cfg.constants.parallel_level <= 1 else -1)
            ...
            if not is_mt:
                ...
                for arm in range(1, K):
                    ...
                    fitted_tester_mt[arm] = fit_drtester_nuisance(
                        model_regression=build_base_learner(cfg.base_learner.type, _best_clf_params_y, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=1 if cfg.constants.parallel_level <= 1 else -1),
                        model_propensity=build_base_learner(cfg.base_learner.type, _best_clf_params_t, seed=cfg.constants.random_seed, task="classifier", parallel_jobs=1 if cfg.constants.parallel_level <= 1 else -1),
    ...
    def run(self, export_bundle: bool | None = None, bundle_dir: str | None = None, bundle_id: str | None = None) -> AnalysisResult:
        ...
        with mlflow.start_run():
            ...
            # Kategorische Features für EconML-Kompatibilität patchen.
            # EconML konvertiert X intern zu numpy → category-Dtype geht verloren.
            # Der Patch stellt sicher, dass LightGBM/CatBoost die kategorialen
            # Spaltenindizes bei jedem .fit()-Aufruf erhalten.
