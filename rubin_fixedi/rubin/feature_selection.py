...
def _causal_forest_importance(
    ...
    try:
        ...
        # Fallback: Spaltenweise konvertieren (robuster bei gemischten Dtypes)
        X_np = np.column_stack([
            pd.to_numeric(X.iloc[:, i], errors="coerce").to_numpy(dtype=np.float64)
            for i in range(X.shape[1])
        ])
        _logger.info("CausalForest: DataFrame-zu-numpy Fallback (spaltenweise Konversion).")
    T_np = np.asarray(T).ravel().astype(np.float64)
    Y_np = np.asarray(Y, dtype=np.float64).ravel()

    # NaN-Check (GRF crasht sonst intern ohne hilfreiche Fehlermeldung)
...
def compute_importances(
    ...
    if parallel_methods and len(effective_methods) >= 2:
        # Prüfe ob CausalForest dabei ist — diese Methode nutzt intern
        # joblib/loky-Prozesse für die Baum-Fits. In einem äußeren
        # joblib-Thread führt das zu fork()-aus-Thread → Deadlock.
        # → CausalForest-Kombinationen IMMER sequentiell laufen lassen.
        has_grf = "causal_forest" in effective_methods
        can_parallelize = not has_grf

        if can_parallelize:
            # Nur LightGBM-Methoden: sicher parallelisierbar (OpenMP ist thread-safe)
            import os
            ...
            # Pre-Import: Module VOR Thread-Start laden (Import-Lock-Sicherheit)
            ...
            _logger.info(
                "Feature-Selektion: %d Methoden sequentiell (CausalForest "
                "nicht mit Thread-Parallelisierung kompatibel — nutzt intern "
                "joblib-Prozesse, die in Threads zu Deadlocks führen).",
