...
Pro Methode werden die Top-X% behalten, dann per Union vereinigt. Anschließend Korrelationsfilter (Pearson + Spearman). Die LightGBM-Fits nutzen automatisch native kategoriale Splits (via `categorical_feature`-Patch). Bei Level 3–4 werden reine LightGBM-Methoden parallel ausgeführt; Kombinationen mit CausalForest laufen sequentiell (GRF nutzt intern joblib-Prozesse, die in Threads zu Deadlocks führen).
