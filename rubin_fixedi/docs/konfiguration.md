...
**Parallelisierung:** Bei `parallel_level` 3–4 werden reine LightGBM-Methoden parallel ausgeführt (z. B. `lgbm_importance` + `lgbm_permutation` gleichzeitig). Kombinationen mit `causal_forest` laufen immer sequentiell — GRF nutzt intern joblib-Prozesse, die in parallelen Threads zu Deadlocks führen. Jede Methode erhält dabei volle CPU-Auslastung (`n_jobs=-1`).
