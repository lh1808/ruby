...
`compute_importances()` parallelisiert Methoden bei Level 3/4 — aber nur wenn **kein** CausalForest beteiligt ist. GRF spawnt intern joblib-Prozesse, die in äußeren joblib-Threads zu Deadlocks führen (fork-aus-Thread-Problem). Kombinationen mit CausalForest laufen immer sequentiell mit voller CPU-Auslastung pro Methode.

Reine LightGBM-Kombinationen (z. B. `lgbm_importance` + `lgbm_permutation`) sind sicher parallelisierbar (OpenMP ist thread-safe).
...
In `analysis_pipeline.run()` werden zwei harmlose Warnungen unterdrückt:
...
Gleicher sklearn-Filter auch in `production_pipeline.__init__()` für Scoring und Explainability.
