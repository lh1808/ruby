...
- `stability_penalty`: Bestraft instabile CATE-Schätzungen mit hoher Streuung relativ zum Median-Effekt. 0.0 = reiner R-Score (Default). 0.3–0.5 = moderate Regularisierung. 1.0+ = starke Stabilität (CATEs werden konservativ, ähnlich CausalForestDML). Formel: `score = R_score − λ · log(1 + CV)`, wobei CV = std(CATE) / |median(CATE)|.
