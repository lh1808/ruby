...
CV = Variationskoeffizient der CATEs (std / |median|). Die logarithmische Dämpfung bestraft Modelle mit extremer Streuung relativ zum mittleren Effekt, ohne stabile Modelle nennenswert einzuschränken. Empfohlene Werte: 0.0 (aus, Default), 0.3 (leicht), 0.5 (moderat), 1.0+ (stark, CATEs werden konservativ ähnlich CausalForestDML).
